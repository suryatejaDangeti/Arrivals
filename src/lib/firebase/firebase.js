...
export const createUserDocumentFromAuth = async (userAuth, additionalInformation = {}) => {
    ...
    if(!userSnapshot.exists()) {
        const { firstName, email, lastName, phoneNumber, postcode, addressLine, city } = userAuth;
...
export const getUserDocumentFromAuth = async (userAuth) => {
    ...
    const userSnapshot = await getDoc(userDocRef);
    const userData = userSnapshot._document.data.value.mapValue.fields
    const transformUserData = (userValues) => {
        return Object.entries(userValues).reduce((acc, [key, value]) => {
            if ( value.stringValue !== "password" ||  value.stringValue !== "confirmPassword" ) {
                acc[key] = value.stringValue || value.timestampValue;
                return acc;
            }
        }, {});
      };
